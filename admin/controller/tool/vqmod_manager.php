<?php
class ControllerToolVQModManager extends Controller {
	/** 
	 * @todo  VQMod 2.3.0
	...
	public function index() {
		$this->language->load('tool/vqmod_manager');
		...
		// Language
		$this->data = array_merge($this->data, $this->language->load('tool/vqmod_manager'));
		...
		$this->data['breadcrumbs'][] = array(
			'href'      => $this->url->link('tool/vqmod_manager', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);

		// Action Buttons
		$this->data['action'] = $this->url->link('tool/vqmod_manager', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['clear_log'] = $this->url->link('tool/vqmod_manager/clear_log', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['clear_vqcache'] = $this->url->link('tool/vqmod_manager/clear_vqcache', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['download_log'] = $this->url->link('tool/vqmod_manager/download_log', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['download_scripts'] = $this->url->link('tool/vqmod_manager/download_vqmod_scripts', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['download_vqcache'] = $this->url->link('tool/vqmod_manager/download_vqcache', 'token=' . $this->session->data['token'], 'SSL');
			...
			foreach ($vqmod_scripts as $vqmod_script) {
				...
				if ($extension == 'xml_') {
					$action[] = array(
						...
						'href' => $this->url->link('tool/vqmod_manager/vqmod_install', 'token=' . $this->session->data['token'] . '&vqmod=' . $file, 'SSL')
					);
				} else {
					$action[] = array(
						'text' => $this->language->get('text_uninstall'),
						'href' => $this->url->link('tool/vqmod_manager/vqmod_uninstall', 'token=' . $this->session->data['token'] . '&vqmod=' . $file, 'SSL')
					);
				}
				...
				$this->data['vqmods'][] = array(
					...
					'delete'      => $this->url->link('tool/vqmod_manager/vqmod_delete', 'token=' . $this->session->data['token'] . '&vqmod=' . basename($vqmod_script), 'SSL'),
					'action'      => $action,
		...
		// Template
		$this->template = 'tool/vqmod_manager.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function vqmod_install() {
		...
		$this->redirect($this->url->link('tool/vqmod_manager', 'token=' . $this->session->data['token'], 'SSL'));
	}
	...
	private function userPermission($permission = 'modify') {
		...
		if (!$this->user->hasPermission($permission, 'tool/vqmod_manager')) {
